<template>
	<view class="u-upload">
		...
		>
			...
			>
				<u-icon class="u-icon" :name="delIcon" :color="delColor"></u-icon>
			</view>
			<u-line-progress
			...
			<view v-if="getFile(item) && !item.isImage">
				<u--image
					@click="doPreviewImage(item.url || item.path, index, item)"
					:width="$u.addUnit(width)"
					:height="$u.addUnit(height)"
		...
		<view
			...
			@tap="selectFile"
			v-if="maxCount > lists.length && !disabled"
		>
			...
			>
				<u-icon name="plus" class="u-add-btn" color="#D3D4D6" size="26"></u-icon>
  ...
  components: {
			...
			handler(val) {
				val?.map((value) => {
					...
					let tmp = this.lists?.some((val) => {
						return val.keyVlaue == value.keyVlauess;
					});
	...
	methods: {
		...
		getFile(item) {
			...
			// 获取后缀名
			let fileExt = item?.file?.type ?? item.fileType;
			// 使用数组的some方法，只要符合limitType中的一个，就返回true
			noArrowExt = ['png', 'jpg', 'jpeg', 'webp', 'gif', 'image', 'image/x-icon'].some(
				(ext) => {
					// 转为小写s
		...
		async uploadFile(index = 0) {
            ...
            if(data && data.code && ![200, 201, 204].includes(data.code)) {
              this.uploadError(index, data);
              return
            }
						// 上传成功
   ...
   async handleDel() {
      ...
      this.delIndex  = index
		...
		handlerDeleteItem(index) {
			...
			const removeFile = this.lists.find((item, findIndex) => findIndex === index);
			this.lists.splice(index, 1);
			this.$forceUpdate();
			this.$emit('on-remove', index, this.lists, this.index, removeFile);
      this.show = false
			this.showToast('移除成功');
      this.formValidate()
		},
		...
		// 预览图片
		doPreviewImage(url, index, item) {
			this.$emit('on-preview', url, this.lists, index, item);
